...
pub struct CostConfig {
    ...
    pub watch_sources: Vec<String>,             // 监听源: ["claude", "codex"]，空=全部
    pub last_sync_time: Option<String>,         // 上次价格同步时间
}
```
...
1. **Cost 区域**
   ...
   - 监听源选择：Claude Code / Codex / Both（控制 watcher 监听哪些日志源）
...
**历史数据处理**：project 字段为空的历史记录视为"无项目归属"。当项目白名单为空时，这些记录参与计算；当项目白名单非空时，空 project 的记录不参与计算（因为无法匹配任何白名单项）。
